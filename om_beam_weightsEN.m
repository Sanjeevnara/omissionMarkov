function om_beam_weightsEN(subJ)
...
%%% various paths ...
fileDir = (['/mnt/obob/staff/gdemarchi/data/markov/decoding/TG_EN_SND/final/']);
outDir  = (['/mnt/obob/staff/gdemarchi/data/markov/decoding/beamweightsEN/final/']);
...
fileToRead  = dir([fileDir subJ '*timegen_EN_SND_plusW_preOMno_Fs100_tWindow0*ICAyes*']);

curFile = fileToRead.name;
tmptg=load([fileDir curFile]);
stuffForWeigths = tmptg.stuffForWeights_trSND_EN;
trainTime= tmptg.time;


%%% for each time point, do the Haufe balancing
for iTime =1:length(trainTime)
